...
NODE1="${NODE1:-https://127.0.0.1:6060}"
NODE2="${NODE2:-https://127.0.0.1:6061}"

die(){ echo "FAIL: $*" >&2; exit 1; }
ok(){ echo "PASS: $*"; }

s1="$(curl -k -fsS "$NODE1/chain/status" | jq -c .)" || die "node1 status down"
s2="$(curl -k -fsS "$NODE2/chain/status" | jq -c .)" || die "node2 status down"
...
c429="$(for i in $(seq 1 200); do curl -k -s -o /dev/null -w '%{http_code}\n' "$NODE1/chain/status"; done | rg -c '^429$' || true)"
[ "${c429:-0}" -gt 0 ] || die "expected 429s on status spam"
ok "read RL works (429s=$c429)"
sleep 2

# Writes not blocked on loopback
pre="$(curl -k -fsS "$NODE1/chain/status" | jq -r .height)"
target=$((pre+10))
for i in $(seq 1 10); do
  for retry in 1 2 3; do
    code="$(curl -k -s -o /dev/null -w '%{http_code}' -X POST "$NODE1/chain/propose_broadcast" || true)"
    [ "$code" = "200" ] && break
    sleep 0.5
  done
...
for tick in $(seq 1 20); do
  h2="$(curl -k -fsS "$NODE2/chain/status" | jq -r .height)"
  [ "$h2" -ge "$target" ] && break
  sleep 1
done
f1="$(curl -k -fsS "$NODE1/chain/status" | jq -c .)"
f2="$(curl -k -fsS "$NODE2/chain/status" | jq -c .)"
